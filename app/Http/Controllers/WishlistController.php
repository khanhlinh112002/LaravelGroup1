...
class WishlistController extends Controller
{
    public function AddWishlist($id)
    {
        if (Session::has('user')) {
            if (Products::find($id)) {
                ...
                echo '<script>alert("Thêm vào wishlist thành công.");window.location.assign("/homepage");</script>';
            } else {
                return '<script>alert("Không tìm thấy sản phẩm này.");window.location.assign("/homepage");</script>';
            }
        } else {
    ...
    public function DeleteWishlist($id)
    {
        ...
        $item->delete();
        return '<script>alert("Đã xóa sản phẩm khỏi wishlist.");window.location.assign("/homepage");</script>';
    }
